...
const imagePath = `${userId}/e2e/${crypto.randomUUID()}${path.extname(inputImage).toLowerCase()}`;
...
const { data: inserted, error: insertError } = await supabase
  ...
  .insert({
    ...
    image_quality_flags: ["original_saved", "display_image_queued", "needs_ai_label_confirmation"],
    category: "待识别",
    color: "待识别",
    fit: "unknown",
    style_tags: ["待识别"],
...
const response = await fetch(`${appUrl.replace(/\/$/, "")}/api/ai/generate-closet-display-image`, {
  method: "POST",
  headers: {
  ...
  body: JSON.stringify({
    ...
    imageDataUrl: imageToDataUrl(inputImage),
  }),
});
...
const { data: verified, error: verifyError } = await supabase
  .from("closet_items")
  .select("id,display_image_path,display_image_status,display_image_model,display_image_prompt_version")
  .eq("id", inserted.id)
  .single();

if (verifyError) throw verifyError;

if (verified.display_image_status !== "ready" || !verified.display_image_path) {
  throw new Error(`Display image was not persisted correctly: ${JSON.stringify(verified)}`);
}

console.log("Closet display route test passed.");
console.log(
  JSON.stringify(
    {
      closetItemId: verified.id,
      displayImageStatus: verified.display_image_status,
      displayImageModel: verified.display_image_model,
      displayImagePromptVersion: verified.display_image_prompt_version,
      hasDisplayImagePath: Boolean(verified.display_image_path),
    },
    null,
    2,
  ),
