...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (screenshotPath) {
      const { error: candidateInsertError } = await supabase.from("purchase_candidates").insert({
...
async function loadRealClosetItems(supabase: SupabaseClient): Promise<ClothingItem[]> {
  ...
  return ((data ?? []) as ClosetRetrievalRow[])
    .filter((item) => item.category && item.category !== "待识别" && item.category !== "识别中")
    .map((item) => ({
      id: item.id,
